...
// cv::cvtColor(number_image, number_image, cv::COLOR_RGB2GRAY);
        ...
        // cv::Mat temp;
        // cv::resize(images[2 - 2 * detect_color], temp, cv::Size(400, 400));
        // ;
...
#ifdef DEBUGNUM
        cv::putText(temp, std::to_string(label_id) + ":" + std::to_string(confidence), cv::Point(50, 50), 1, 2, cv::Scalar(255, 255, 255), 2);
        cv::imshow("num" + std::to_string(i++), temp);
#endif
