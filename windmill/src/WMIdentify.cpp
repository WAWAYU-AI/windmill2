 ...
 */
 ...
 void WMIdentify::identifyWM(cv::Mat &input_img, Translator &translator) {
   ...
   result = detect(this->img, blade, *(this->gp), translator.message.status / 5);
   ...
   if (blade.apex.size() == 6) {
    ...
    //  translator.message.armor_flag = 11;
     ...
     if (R_world_coordinate_car.at<double>(3, 0) != 0) { // 检查齐次坐标w分量
       R_car =
           cv::Point3f(R_world_coordinate_car.at<double>(0, 0) / R_world_coordinate_car.at<double>(3, 0),
                       R_world_coordinate_car.at<double>(1, 0) / R_world_coordinate_car.at<double>(3, 0),
                       R_world_coordinate_car.at<double>(2, 0) / R_world_coordinate_car.at<double>(3, 0));
     }
     ...
     if (Top_car_mat.at<double>(3, 0) != 0) { // 检查齐次坐标w分量
       Top_car =
           cv::Point3f(Top_car_mat.at<double>(0, 0) / Top_car_mat.at<double>(3, 0),
 ...
 cv::Mat WMIdentify::calculateTransformationMatrix(cv::Mat R_world2cam,
   ...
   double tx_cam2cloud = this->gp->tx_cam2cloud;
   double ty_cam2cloud = this->gp->ty_cam2cloud;
   double tz_cam2cloud = this->gp->tz_cam2cloud;
