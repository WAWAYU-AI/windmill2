...
void WMIdentify::identifyWM(cv::Mat &input_img, Translator &translator) {
  ...
  if (blade.apex.size() == 6) {
    ...
    cv::Point3f R_world = cv::Point3f(this->gp->d_RP2, 0, 0);
    // 将Point3f转换为Mat格式(4x1矩阵,齐次坐标)
    cv::Mat R_world_mat =
        (cv::Mat_<double>(4, 1) << R_world.x, R_world.y, R_world.z, 1.0);

    // 进行矩阵乘法
    cv::Mat R_car_mat = this->world2car * R_world_mat;

    // 将结果转换回Point3f
    cv::Point3f R_car;
    if (R_car_mat.at<double>(3, 0) != 0) { // 检查齐次坐标w分量
      R_car =
          cv::Point3f(R_car_mat.at<double>(0, 0) / R_car_mat.at<double>(3, 0),
                      R_car_mat.at<double>(1, 0) / R_car_mat.at<double>(3, 0),
                      R_car_mat.at<double>(2, 0) / R_car_mat.at<double>(3, 0));
    }
    ...
    LOG_IF(INFO, this->switch_INFO) << "sita :" << 180 * theta / 3.14;
    ...
    if (angle_list.size() >= gp->list_size - 2) {
      ...
      this->s =
          sqrt(cv::norm(tvec) * cv::norm(tvec) - this->gp->H_0 * this->gp->H_0);
...
cv::Mat WMIdentify::calculateTransformationMatrix(cv::Mat R_world2cam,
  ...
  double tx_cam2cloud = 0.0;
  double ty_cam2cloud = -0.36;
...
double WMIdentify::calculatePhi(cv::Mat rotation_matrix, cv::Mat tvec) {
  ...
  // Vz) * sqrt(Zx * Zx + Zz * Zz)) = "<< Vx * Zx + Vz * Zz / (sqrt(Vx * Vx + Vz
  // * Vz) * sqrt(Zx * Zx + Zz * Zz));
  return Pi - phi;
}
/**
